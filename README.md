...
Бот работает на **Gemini** (когда задан `GEMINI_API_KEY`; модель — `GEMINI_MODEL`, по умолчанию `gemini-2.5-flash`) или на **Claude** (`ANTHROPIC_API_KEY`) как запасном варианте. Промпт, инструменты и формат истории общие — провайдера можно переключить без потери диалогов.
